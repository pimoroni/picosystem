...
mp_obj_t picosystem_pressed(mp_obj_t b_obj) {
    ...
    switch(b) {
        ...
        default:
            mp_raise_ValueError(MP_ERROR_TEXT("pressed(): invalid button."));
            break;
    }
...
mp_obj_t picosystem_button(mp_obj_t b_obj) {
    ...
    switch(b) {
        ...
        default:
            mp_raise_ValueError(MP_ERROR_TEXT("button(): invalid button."));
            break;
    }
...
mp_obj_t picosystem_led(mp_obj_t r_obj, mp_obj_t g_obj, mp_obj_t b_obj) {
    ...
    if(r < 0 || r > 100) {
        mp_raise_ValueError(MP_ERROR_TEXT("led(): r out of range. Expected 0 to 100"));
    }
    else if(g < 0 || g > 100) {
        mp_raise_ValueError(MP_ERROR_TEXT("led(): g out of range. Expected 0 to 100"));
    }
    else if(b < 0 || b > 100) {
        mp_raise_ValueError(MP_ERROR_TEXT("led(): b out of range. Expected 0 to 100"));
    }
    else {
        led(r, g, b);
    }
...
mp_obj_t picosystem_backlight(mp_obj_t b_obj) {
    ...
    if(b < 0 || b > 100) {
        mp_raise_ValueError(MP_ERROR_TEXT("backlight(): b out of range. Expected 0 to 100"));
    }
    else {
        backlight(b);
    }
