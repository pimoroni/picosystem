...
mp_obj_t picosystem_rgb(mp_uint_t n_args, const mp_obj_t *args) {
    ...
    if(r < 0 || r > 15) {
        mp_raise_ValueError(MP_ERROR_TEXT("rgb(): r out of range. Expected 0 to 15"));
    }
    else if(g < 0 || g > 15) {
        mp_raise_ValueError(MP_ERROR_TEXT("rgb(): g out of range. Expected 0 to 15"));
    }
    else if(b < 0 || b > 15) {
        mp_raise_ValueError(MP_ERROR_TEXT("rgb(): b out of range. Expected 0 to 15"));
    }
    else {
        if(n_args == 4) {
            int a = mp_obj_get_int(args[3]);
            if(a < 0 || a > 15) {
                mp_raise_ValueError(MP_ERROR_TEXT("rgb(): a out of range. Expected 0 to 15"));
            }
            else {
...
mp_obj_t picosystem_hsv(mp_uint_t n_args, const mp_obj_t *args) {
    ...
    if(h < 0.0f || h > 1.0f) {
        mp_raise_ValueError(MP_ERROR_TEXT("hsv(): h out of range. Expected 0.0 to 1.0"));
    }
    else if(s < 0.0f || s > 1.0f) {
        mp_raise_ValueError(MP_ERROR_TEXT("hsv(): s out of range. Expected 0.0 to 1.0"));
    }
    else if(v < 0.0f || v > 1.0f) {
        mp_raise_ValueError(MP_ERROR_TEXT("hsv(): v out of range. Expected 0.0 to 1.0"));
    }
    else {
        if(n_args == 4) {
